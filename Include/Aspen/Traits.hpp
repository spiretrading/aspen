...
namespace Aspen {
   ...
   * @param <T> The type of reactor to evaluate.
   */
  template<typename T>
  struct reactor_result {
    using type = typename to_reactor_t<T>::Type;
  };
  ...
  /**
   * Trait used to determine how a value is returned by an evaluation.
   ...
   * @param <R> The type of reactor to evaluate.
   */
   ...
   * @param <R> The types of reactors to evaluate.
   */
  template<IsReactor... R>
   ...
   */
  template<IsReactor... R>
  using common_evaluation_t = std::conditional_t<
    (is_reference_evaluation_v<R> && ...) &&
      (std::same_as<reactor_result_t<R>, common_result_t<R...>> && ...),
  ...
  constexpr auto is_noexcept_reactor_v =
    (noexcept(std::declval<const R&>().eval()) && ...);
